...
export function InspectionTime({ phase = 'pickup', accessoryCount = 0 }: { phase?: InspectionPhase; accessoryCount?: number }) {
  return <aside className="time-estimate" aria-label="Estimated inspection time">
    <Clock3 size={21} aria-hidden="true" />
    <div><p><strong>{phase === 'pickup' ? 'Pickup check · ' + minutesLabel(fullInspectionMinutes) : 'After delivery · first week'}</strong><span>{phase === 'pickup' ? '20 core checks' + (accessoryCount ? ` + ${accessoryCount} ${accessoryCount === 1 ? 'accessory' : 'accessories'}` : '') : '10 follow-up checks'}</span></p>
