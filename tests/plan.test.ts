...
assert.equal(pickupIds.length, 20); assert.equal(deliverySections.length, 7);
...
assert.deepEqual(deliverySections.map(s=>s.id), ['route-front','route-passenger','route-rear','route-driver','route-cabin','route-seat','route-handover']);
...
assert.equal(pickupSections(equipped)[2].items[2].id, extras[0].id, 'Individual items appear directly after the equipment check');
...
assert.ok(recordSections(equipped)[2].items.some(item => item.id === extras[1].id));
...
assert.equal(pickupSections(accessoryMerge.merged)[2].items.length, 6);
